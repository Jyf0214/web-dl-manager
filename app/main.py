...
USERNAME = "Jyf0214"
GALLERY_DL_PASSWORD = os.getenv("GALLERY_DL_PASSWORD")
PASSWORD_HASH = pwd_context.hash(GALLERY_DL_PASSWORD) if GALLERY_DL_PASSWORD else None
...
app.mount("/static_site", StaticFiles(directory=STATIC_SITE_DIR), name="static_site")
...
async def run_command(command: str, command_to_log: str, status_file: Path):
    """Runs a shell command asynchronously, logs its progress in real-time."""
    with open(status_file, "a") as f:
...
async def upload_to_gofile(file_path: Path, status_file: Path) -> str:
    ...
    for attempt in range(max_retries):
        try:
            with open(status_file, "a") as f:
                f.write(f"Uploading {file_path.name} to gofile.io (Attempt {attempt + 1}/{max_retries})...\n")

            async with httpx.AsyncClient(timeout=600.0) as client: # 10 minute timeout for upload
            ...
            with open(status_file, "a") as f:
                f.write(f"Upload attempt {attempt + 1} failed: {e}\n")
            if attempt < max_retries - 1:
                await asyncio.sleep(10) # Wait 10 seconds before retrying
            else:
                raise Exception("Failed to upload to gofile.io after multiple retries.")
...
async def process_download_job(task_id: str, url: str, downloader: str, service: str, upload_path: str, params: dict):
    ...
    try:
        ...
        compress_cmd = f"tar -cf - -C {source_to_compress} . | zstd | split -b 1500M - \"{task_archive_path}.part\""
        await run_command(compress_cmd, compress_cmd, status_file)

        # 3. Upload
        archive_parts = sorted(list(ARCHIVES_DIR.glob(f"{archive_name}.tar.zst.part*")))
        if not archive_parts:
            raise FileNotFoundError("Archive splitting failed, no parts found.")

        for part_path in archive_parts:
            if service == "gofile":
                await upload_to_gofile(part_path, status_file)
            else:
                rclone_config_path = create_rclone_config(task_id, service, params)
                remote_full_path = f"remote:{upload_path}"
                upload_cmd = (
                    f"rclone copyto --config \"{rclone_config_path}\" \"{part_path}\" \"{remote_full_path}/{part_path.name}\" "
...
async def get_index(request: Request):
    ...
    if PRIVATE_MODE and not is_authenticated(request):
        # Check if static_wrapper.html exists, otherwise fallback to gate.html
        static_wrapper_path = STATIC_SITE_DIR / "static_wrapper.html"
        if static_wrapper_path.exists():
            with open(static_wrapper_path, "r") as f:
                content = f.read()
    ...
    if os.path.exists(STATUS_DIR):
        tasks = os.listdir(STATUS_DIR)

    return templates.TemplateResponse("index.html", {"request": request, "lang": lang, "authenticated": is_authenticated(request), "tasks": tasks, "username": "Jyf0214"})
...
async def post_login(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if username == USERNAME:
        if PASSWORD_HASH:
            ...
            # Password is not set, username is enough
            user_authenticated = True
    ...
    return templates.TemplateResponse("login.html", {"request": request, "lang": lang, "error": "Invalid username or password"})
...
async def get_status(request: Request, task_id: str):
    ...
    with open(status_file, "r") as f:
        content = f.read()
        gofile_links = re.findall(r"Download link: (https://gofile\\.io/\\S+)", content)
        
    return templates.TemplateResponse(
        "status.html", 
        {"request": request, "task_id": task_id, "log_content": content, "lang": lang, "gofile_links": gofile_links}
    )
