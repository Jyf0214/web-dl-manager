...
-   **用户认证与博客:**
    -   **私有模式:** 使用登录页面以私有模式运行应用程序。
    -   **可自定义用户:** 通过环境变量 (`APP_USERNAME`, `APP_PASSWORD`, `AVATAR_URL`) 设置用户名、密码（可选）和头像。
    -   **博客索引:** 在根 URL 显示来自 Git 仓库的静态博客。
    -   **任务查看器:** 在私有模式下，已登录用户可以在 `/tasks` 页面上查看所有任务。
...
-   **通用免责声明:** 本工具按“原样”提供，不作任何保证。用户对使用此应用程序下载的任何内容负全部责任。请尊重您要下载网站的服务条款以及内容创建者的知识产权。
